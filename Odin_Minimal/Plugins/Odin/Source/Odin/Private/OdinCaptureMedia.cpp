...
void UOdinCaptureMedia::SetAudioCapture(UAudioCapture* audio_capture)
{
    ...
    if (audio_capture && audio_capture->IsValidLowLevel()) {
        ...
        TFunction<void(const float* InAudio, int32 NumSamples)> audioGeneratorDelegate =
            [World, this](const float* InAudio, int32 NumSamples) {
                if (bIsBeingReset) {
                    return;
                }

                if (!UOdinFunctionLibrary::OdinAsyncValidityCheck(
                        World, this, "OdinCaptureMedia: Audio Generator Delegate"))
                    return;

                if (!audio_capture_ || !audio_capture_->IsValidLowLevel()) {
                    UE_LOG(Odin, Warning,
...
void UOdinCaptureMedia::HandleInputDeviceChanges()
{
    ...
    AsyncTask(ENamedThreads::GameThread, [World, this]() {
        if (!UOdinFunctionLibrary::OdinAsyncValidityCheck(
                World, this, "UOdinCaptureMedia: HandleInputDeviceChanges"))
            return;
